...
def asylmat(A,B,m,c,x1=0,y1=0):
    ...
    if c == "X":
        ...
        for i in range(1,y2):
            ...
            C[2*n**2+y1+i][n**2+(n-y2)*n+n-y2] = 1
            C[2*n**2+y1+i][n**2+(n-y2+i)*n+n-y2+i] = 1            
        for i in range(y1):
            l.remove(i*n+i)
        for i in range(y1,n-y2):
...
def lemma_3_blocks(n,m,n1,n2):
    M = ran(n,m,"G")
    a = ran(n,m,"A",x1=n1)
    a = conj(a,M,m)
    n = findcom(a,m,"A",x1=n2)[0]
    return n == 1

def experiment_5_4(n,m,n1,n2):
    k1 = max(n1,n2)
    k2 = min(n1,n2)
    M1 = ran(n,m,"G")
    M2 = ran(n,m,"G")
    a = conj(ran(n,m,"A",x1=k1),M1,m)
    b = conj(ran(n,m,"A",x1=k2),M2,m)
    x1 = conj(ran(n,m,"Y",x1=k2),M2,m)
    x2 = conj(ran(n,m,"Y",x1=k2),M2,m)
    y1 = conj(ran(n,m,"Y",x1=k1),M1,m)
    y2 = conj(ran(n,m,"Y",x1=k1),M1,m)
    K = com(a,b,m)
    u = longmul([inv(x1,m),a,x2],m)
    p = longmul([inv(y1,m),b,y2],m)
    ta = conj(inv(p,m),a,m)
    tb = conj(u,b,m)
    left = conj(u,inv(M2,m),m)
    right = conj(tb,inv(M2,m),m)
    ...
    left = conj(b_prime,inv(M1,m),m)
    right = conj(p,inv(M1,m),m)
    y2_prime,y1_prime = asyl(left,right,m,"Y",x1=k1)
    y2_prime = conj(y2_prime,M1,m)
    y1_prime = conj(y1_prime,M1,m)
    K_prime = longmul([y2_prime,ta,inv(y1_prime,m),b_prime],m)
    return (K==K_prime).all()
